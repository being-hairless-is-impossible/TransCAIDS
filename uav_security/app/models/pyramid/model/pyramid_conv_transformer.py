...
# Updated Model with Innovations
class PyramidTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.fc = nn.Sequential(
            nn.Linear(128 * 2, 256),
            nn.BatchNorm1d(256),  # Batch normalization here
            nn.ReLU(),
            self.dropout,
    ...
    def forward(self, x_cyber, x_physical):
        # Cyber branch
        x_cyber = self.conv1_cyber(x_cyber.unsqueeze(1))  # (batch, 1, seq_len) -> (batch, 64, seq_len/2)
        x_cyber = self.pyramid_cyber(x_cyber)
        x_cyber = self.connect_cyber(x_cyber)  # (batch, 128, seq_len/2)

        # Prepare for Cyber for transformer: (batch, seq_len, features)
        x_cyber = x_cyber.transpose(1, 2)  # (batch, seq_len, 128)
        ...
        # Physical branch
        x_physical = self.conv1_physical(x_physical.unsqueeze(1))  # (batch, 1, seq_len) -> (batch, 64, seq_len/2)
        x_physical = self.dilated_physical(x_physical)
        x_physical = self.connect_physical(x_physical)  # (batch, 128, seq_len/2)

        # Prepare for Physical transformer: (batch, seq_len, features)
        x_physical = x_physical.transpose(1, 2)  # (batch, seq_len, 128)
        ...
        # Pooling
        x_cyber_pooled = torch.mean(x_cyber_attn, dim=1)  # (batch, 128)
        x_physical_pooled = torch.mean(x_physical_attn, dim=1)  # (batch, 128)

        # Concat
        x_concat = torch.cat((x_cyber_pooled, x_physical_pooled), dim=1)  # (batch, 256)

        # fc for connecting layer
